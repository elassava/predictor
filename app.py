...
def predict():
    try:
        ...
        logger.info(f"Making prediction for symbol: {selected_symbol}")
        
        # Veri işleme
        df_daily = fetch_yfinance_data(selected_symbol).copy()
        logger.info(f"Fetched data shape: {df_daily.shape}")
        
        if df_daily.empty:
            logger.error("No data received from yfinance")
            return jsonify({'error': 'No data available for the selected symbol'}), 400
        ...
        if len(x_daily) == 0 or len(y_daily) == 0:
            logger.error("Empty dataset created")
            return jsonify({'error': 'Could not create dataset for prediction'}), 400
